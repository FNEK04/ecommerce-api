...
- **Корзины** (Cart)
- **Заказы** (Order)
- **Способы оплат** (PaymentMethod)

### Основной функционал
- Авторизация пользователя (Laravel Sanctum)
- Одна корзина на пользователя (hasOne)
- Корзина содержит множество товаров (CartItem)
- Оформление заказа из корзины, удаление корзины после оплаты
- Статусы заказа: "На оплату", "Оплачен", "Отменен"
- Генерация уникальной ссылки на оплату для каждого заказа
- Callback по ссылке оплаты для смены статуса заказа
- Автоматическая отмена заказа через 2 минуты (фоновая задача)
- История заказов пользователя, фильтрация и сортировка

### REST API
- Авторизация: регистрация, вход, выход
- Товары: список, просмотр, сортировка по цене
- Корзина: добавление, удаление, просмотр
- Заказы: оформление, оплата, история, фильтрация, сортировка
- Callback оплаты: обновление статуса заказа

### Фоновые задачи
- Команда для отмены просроченных заказов (Kernel, schedule)

### Покрытие тестами
- Unit и Feature тесты (php artisan test)
- Покрытие > 50%
...
- **Laravel 10+**
- Используется Laravel Sanctum для API-авторизации
- Все бизнес-сценарии покрыты feature- и unit-тестами
- Миграции, фабрики, сидеры для всех сущностей
- Чистая структура контроллеров, моделей, ресурсов
- Вся логика строго по ТЗ, без лишних зависимостей
- Первый коммит — "голый" Laravel, далее — поэтапная реализация (см. git log)

---

## Как запустить
1. `composer install`
2. `cp .env.example .env` и настройте параметры БД
3. `php artisan migrate --seed`
4. `php artisan serve`
5. Для фоновых задач: `php artisan schedule:work` или cron
